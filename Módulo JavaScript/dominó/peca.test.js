...
test("criar peça 7/1 resulta em erro.", assert =>{
    assert.throwsWithMessage(PecaError, `O número máximo de valor para cada lado é ${Peca.VALOR_MAXIMO}. `, () => {
        const peca = new Peca(7, 1)
    })
})

test("criar peça -1/4 resulta em erro.", assert =>{
    assert.throwsWithMessage(PecaError, `O número mínimo de valor para cada lado é ${Peca.VALOR_MINIMO}. `, () => {
